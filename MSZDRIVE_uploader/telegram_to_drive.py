...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--index", default="", help="Editable text folder index generated by telegram_folder_index.")
    parser.add_argument("--topic-link", default="", help="Telegram topic link for flat no-index uploads.")
    parser.add_argument(
        "--flat-folder",
        default="",
        help="Upload all Telegram media into this one folder. Use __topic_title__ to resolve the topic title.",
    )
...
async def _resolve_topic_title(telegram, parsed) -> str:
    try:
        title = await telegram.get_forum_topic_title(parsed.chat_id, parsed.topic_id)
    except Exception:
        title = None
    if title:
    ...
    try:
        message = await telegram.get_message(parsed.chat_id, parsed.message_id)
    except Exception:
        message = None
    ...
    text = " ".join(part.strip() for part in str(text).splitlines() if part.strip())
    return text.strip()


def _safe_folder_name(value: str, fallback: str) -> str:
    cleaned = Path(value.replace("\x00", "")).name.strip().strip(" .")
...
def _media_folder_assignments(
    ...
    *,
    above: bool,
    default_folder: str = UNSORTED_FOLDER,
) -> list[tuple[object, str]]:
    assignments: list[tuple[object, str]] = []
    default_folder = norm_rel(default_folder) or UNSORTED_FOLDER
    if not above:
    ...
    if pending_media:
        print(f"[folder above] {len(pending_media)} trailing media had no following heading; using {default_folder}", flush=True)
        assignments.extend((media, default_folder) for media in pending_media)
    return assignments
...
async def run(args: argparse.Namespace) -> int:
    folder_index = parse_index(Path(args.index)) if args.index else None
    if folder_index is None and not (args.topic_link and args.flat_folder):
        raise ValueError("--index is required unless --topic-link and --flat-folder are provided.")

    topic_link = folder_index.topic_link if folder_index is not None else args.topic_link
    root_folder = _topic_root(folder_index) if folder_index is not None else ""
    paths_by_heading = (
        _root_heading_paths(folder_paths_by_heading(folder_index.headings), root_folder)
        if folder_index is not None
        else {}
    )
    heading_ids = {heading.message_id for heading in folder_index.headings} if folder_index is not None else set()
    default_folder = args.flat_folder if folder_index is None else UNSORTED_FOLDER
    if args.target in {"msz", "both"} and not args.target_folder and not default_folder:
    ...
    parsed = parse_export_link(topic_link)
    if not parsed.is_topic:
        raise ValueError("Telegram uploads require a private forum topic link.")
    start_message_id = (folder_index.start_message_id if folder_index is not None else 0) or parsed.topic_id
    ...
    try:
        ...
        if args.tg_download == "hyper" and not telegram.helper_clients:
            ...
            return 1
        if default_folder == "__topic_title__":
            topic_title = await _resolve_topic_title(telegram, parsed)
        ...
        for message, active_folder in assignments:
            ...
            try:
                ...
                file_name = telegram_media_filename(message, use_caption=args.caption_file_names)
                if not Path(file_name).suffix:
                    inferred = infer_extension_from_signature(local_path)
                    if inferred:
                        file_name += inferred
