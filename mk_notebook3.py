...
async def uploader_task_loop(app, output_dir, chat_id, topic_id, download_process_event):
    ...
    while not download_process_event.is_set():
        ...
        for f in files:
            path_str = str(f)
            # Ignore temp files (JD2 often appends .part)
...
async def download_task(url, output_dir, download_process_event):
    ...
    if JD_EMAIL and JD_PASS and not USE_ARIA2_FALLBACK:
        ...
        # Keep checking directory size to guess completion, 
        # or user might manually add files from website.
        # So we leave JD2 open for a considerable idle time (e.g. 5 minutes no activity)
        prev_size = -1
        stagnant_count = 0
        while True:
            ...
            if size == prev_size and size > 0:
                ...
                stagnant_count = 0
            prev_size = size
            # Close JD2 if files are downloaded and nothing changed in 100 seconds (10 loops).
            # If size == 0, it means it hasn't caught the URL yet (we wait up to 10 minutes (60 loops))
...
print("Updated JDownloader2_Leecher.ipynb with Website Auth successfully!")
