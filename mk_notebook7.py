...
# @markdown Specify the path to your `.txt` config file containing `API_ID`, `API_HASH`, `USER_SESSION_STRING`, `JD_EMAIL`, and `JD_PASS`. By default, it expects a `config.txt` file in the root of your mounted Google Drive.
ENV_FILE_PATH = "/content/drive/MyDrive/config.txt" # @param {type:"string"}

# @markdown ---
# @markdown ### ⚙️ Advanced JD2 Configuration
# @markdown If JD credentials are in your `.txt` config, this registers the Colab instance on [MyJDownloader](https://my.jdownloader.org/) to remote manage Premium Accounts and links while the cell runs.
...
if ENV_FILE_PATH.startswith("/content/drive"):
    print(f"[*] Mounting Google Drive to access config file...")
...
print("Updated JDownloader2_Leecher.ipynb for config.txt support successfully!")
